...
class PlannerService:
    ...
    async def generate_plan(
        ...
        return self._build_response(plan, events_data, body, saved=len(events_rows), all_events_rows=events_rows)
    ...
    def _build_response(
        ...
        *,
        saved: int,
        all_events_rows: list | None = None,
    ) -> dict[str, Any]:
        # Build lookup from filtered events first, then supplement with all rows
        # so cached plans can still enrich items filtered out by current_time.
        events_by_id = {str(e["id"]): e for e in events_data}
        if all_events_rows:
            for row in all_events_rows:
                eid = str(row["id"])
                if eid not in events_by_id:
                    events_by_id[eid] = {
                        ...
                        "location": row["location_name"] or "",
                        "coords": (
                            {"lat": row["venue_lat"], "lng": row["venue_lng"]}
                            if row["venue_lat"] and row["venue_lng"]
                            else None
                        ),
                        "category": row["category"] or "Other",
                        "price_thb": row["price_thb"],
        ...
        for item in plan.get("skipped", []):
            ...
            if ev:
                ...
                item["price_thb"] = ev.get("price_thb")
